...
fn main() {
    ...
    let (arch, vendor, sys, _abi) = (target_s.next().unwrap(), target_s.next().unwrap(),
                                     target_s.next().unwrap(), target_s.next().unwrap_or(""));
    let mut lib_dir = match env::var("CUBISM_CORE") {
        Ok(dir) => PathBuf::from(dir),
        Err(_) => panic!("The CUBISM_CORE environment variable hasn't been set! \
